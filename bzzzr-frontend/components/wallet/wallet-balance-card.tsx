...
export function WalletBalanceCard({ balance, walletAddress, isConnected = false }: WalletBalanceCardProps) {
  ...
  return (
    <div className="bg-[var(--bg-secondary)] rounded-xl p-4 card-shadow text-center">
      <h2 className="text-xl font-bold text-[var(--text-primary)] mb-2">{t("walletPage.myBalance")}</h2>

      {isConnected ? (
        <>
          <div className="flex items-center justify-center gap-2 mb-4">
            <TonIcon width={32} height={32} className="text-[var(--accent-gold)]" />
            <span className="text-4xl font-bold text-[var(--text-primary)]">{balance.toFixed(2)}</span>
          </div>

          <div className="grid grid-cols-4 gap-2 mb-4">
            <Button variant="secondary" size="icon-lg" onClick={handleDeposit}>
              <Download className="w-6 h-6 text-[var(--accent-gold)]" />
              <span className="sr-only">{t("walletPage.deposit")}</span>
            </Button>
            <Button variant="secondary" size="icon-lg" onClick={handleWithdraw}>
              <Upload className="w-6 h-6 text-[var(--accent-gold)]" />
              <span className="sr-only">{t("walletPage.withdraw")}</span>
            </Button>
            <Button variant="secondary" size="icon-lg" onClick={handleInviteFriend}>
              <UserPlus className="w-6 h-6 text-[var(--accent-gold)]" />
              <span className="sr-only">{t("walletPage.inviteFriend")}</span>
            </Button>
            <Button variant="secondary" size="icon-lg" onClick={handleCopyReferralLink}>
        ...
        <>
          ...
          <p className="text-sm text-[var(--text-secondary)]">{t("walletPage.connectWalletDescription")}</p>
        </>
      )}
