...
const mockDailyTasks = [
  { points: 10, description: "Buy/sold items (for every 10 TON)", status: "check" as const, isDaily: true },
    ...
    <div className="flex-1 rounded-xl p-4 gradient-bg-marketplace-page p-4 space-y-6">
      ...
      <div className="bg-[var(--bg-secondary)] rounded-xl p-4 card-shadow">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-[var(--text-primary)]">{t("tasksPage.battle")} 1</h2>
        ...
        <div className="relative bg-[var(--bg-tertiary)] rounded-lg p-4 flex items-center justify-between">
          <div className="absolute -top-4 -left-4 w-16 h-16 bg-gradient-gold rounded-full opacity-10" />
          <div className="absolute -bottom-4 -right-4 w-16 h-16 bg-gradient-gold rounded-full opacity-10" />
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          <h3 className="text-lg font-semibold text-[var(--text-primary)]">{t("tasksPage.onetime")}</h3>
          {mockOnetimeTasks.map((task, index) => (
            <TaskItem key={index} {...task} />
          ))}
        </div>
        <div className="space-y-3">
          <h3 className="text-lg font-semibold text-[var(--text-primary)]">{t("tasksPage.dailyTasks")}</h3>
